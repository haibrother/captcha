...
class SimpleCaptcha {
    ...
    public function CreateImage() {
       ...
       // $_SESSION[$this->session_var] = $text;
        if(isset($_COOKIE['captcha']) && !empty($_COOKIE['captcha'])){
            apc_store($_COOKIE['captcha'],$text,600);
        }else{
            $lib = new library();
            $ip = $lib->get_client_ip();
            $key = $ip.'_'.time();
            setcookie("captcha", $key, time()+3600,'/','.hx9999.com');
            apc_store($key,$text,600);
        }
